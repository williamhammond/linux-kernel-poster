...
# Top level targets.
echo 'image.ps: outline.ps ../key.ps ../linuxcare.ps ../rusty.ps ../place.sh ring1.ps ring2.ps ring3.ps ring4.ps'
echo '	cat ring[1234].ps > $@'
echo '	../place.sh below ../linuxcare.ps $@ >> $@'
echo '	../place.sh bottom-right ../rusty.ps $@ >> $@'
...
echo '	@../total_area.sh `find $* -name \*-all.ps` > $@'
...
echo %.ps::%
echo '	@../analyze_function $< | ../data2ps `echo $< | sed '\''s/^.*\.\([^.]*\)\.\+.*$$/\1/'\''` > $@'
echo

# Rule to make all from constituent parts.
echo %-all.ps:
echo "	@echo Rendering \$*.c"
echo "	@../conglomerate_functions.sh $FUNCTION_SCRUNCH \$^ > \$@"
echo
...
for d in `find . -type d`; do
    for f in `cd $d; ls *+ 2>/dev/null | sed 's/\.c\..*+$//' | uniq`; do
	echo $d/$f-all.ps: `cd $d; ls $f.c.*+ | sed -e 's/$/.ps/' -e "s?^?$d/?"`
    done
done
